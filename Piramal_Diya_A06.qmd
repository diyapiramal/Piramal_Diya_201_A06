...
[https://github.com/diyapiramal/Piramal_Diya_201_A06](https://github.com/diyapiramal/Piramal_Diya_201_A06)
...
for (initials in names(player_info)) {
  player_info[[initials]]$height <- height[[initials]]
}
...
for (initials in names(player_info)) {
  player <- player_info[[initials]]
  points_per_game[[initials]] <- player$points / player$games_played
}
...
```{r}
# Round the points per game to two decimal places
rounded_points_per_game <- sapply(points_per_game, round, digits = 2)
...
rounded_mat <- apply(mat_rand, 1:2, round_fun)
rounded_mat
```
...
```{r}
mean_temp <- apply(max_temps [,3:14], 2, mean)
...
```{r}
annual_temp <- apply(colorado_temp[, 3:14], 1, median)

max_temp <- which.max(annual_temp)
min_temp <- which.min(annual_temp)

max_year <- colorado_temp$year[max_temp]
min_year <- colorado_temp$year[min_temp]

print(paste0("The maximum median annual temperature of ", annual_temp[max_temp], " occured in ", max_year, " and the minimum median annual temperature of ", annual_temp[min_temp], " occured in ", min_year, "."))
...
sum_of_digits <- function(n) {
  ...
  # last digit of the number input
  last_digit = n %% 10

  #truncating the number 
  number_truncated = n %/% 10

  return(last_digit + sum_of_digits(number_truncated))
  }
}
